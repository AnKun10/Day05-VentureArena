...
- Secrets: `OPENAI_API_KEY`, `TAVILY_API_KEY`, `DISCORD_TOKEN` qua `.env` (gitignore); template commit sẵn tại `codebase/backend/.env.example`. Kênh nguồn: mặc định chỉ cần `DISCORD_GUILD_ID` — DiscordSource tự resolve 3 kênh forum theo tên (normalize bỏ emoji + dấu tiếng Việt rồi match `chia-se`/`bai-hoc`/`tai-nguyen`); 3 biến `DISCORD_CHANNEL_*` là override khi tên kênh khác chuẩn.
